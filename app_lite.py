...
def check_or_request_license() -> bool:
    ...
    if os.path.exists(license_file_path):
        ...
        if verify_key(key):
            logging.info("License hợp lệ được tìm thấy."); return True
        else:
            logging.warning("File license không hợp lệ, đang xóa."); os.remove(license_file_path)
    while True:
...
class PracticeLiteWindow(QMainWindow):
    ...
    def start_camera(self):
        ...
        logging.info(f"Phát hiện {num_cameras} camera.")
        if num_cameras < 2:
            self.disconnect_camera("Vui lòng kết nối USB camera và nhấn 'Làm mới'"); return
        cam_index = self.config.get('camera_index', 1)
        logging.info(f"Đang thử kết nối với camera index {cam_index} từ file config...")
        self.disconnect_camera()
        self.cam = Camera(cam_index)
        if not self.cam.isOpened():
            error_msg = f"Lỗi: Không thể mở Camera index {cam_index}.\nHãy kiểm tra lại file config.json."
            self.disconnect_camera(error_msg); QMessageBox.warning(self, "Lỗi kết nối", error_msg); return
        is_ok, _ = self.cam.read()
        if is_ok:
            self.video_timer.start(30); self.is_camera_connected = True; self.trigger.activate()
            logging.info(f"Đã kết nối thành công tới camera index {cam_index}.")
        else:
    ...
    def disconnect_camera(self, message="Vui lòng kết nối USB camera và nhấn 'Làm mới'"):
        ...
        self.cam = None; self.is_camera_connected = False; self.gui.camera_view_label.setText(message)
    ...
    def draw_custom_reticle(self, image, center_point):
        ...
        for i in range(1, num_major_ticks + 1):
            dist = (i * axis_len) // num_major_ticks
            major_tick_size = 8
            minor_tick_size = 4
    ...
    def update_frame(self):
        ...
        if not ret or frame is None:
            self.disconnect_camera("Mất kết nối camera."); return
    ...
    def toggle_calibration_mode(self, force_off=False):
        ...
        if new_state_is_on:
            QMessageBox.information(self, "Hiệu chỉnh", "Click vào vị trí tâm ngắm mong muốn trên màn hình camera.")
        elif not force_off:
            self.calibrated_center = None; logging.info("Người dùng đã hủy hiệu chỉnh. Reset tâm ngắm.")
